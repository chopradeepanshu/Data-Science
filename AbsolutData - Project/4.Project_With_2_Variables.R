...
Hemoglobin.Outliers <-
  boxplot.stats(patientData$Level_of_Hemoglobin)$out  # outlier values.
boxplot(patientData$Level_of_Hemoglobin,
        main = "Hemoglobin Data",
        boxwex = 1)
mtext(paste("Outliers: ", paste(Hemoglobin.Outliers, collapse = ", ")), cex =
        1)


Genetic_Pedigree_Coefficient.Outliers <-
  boxplot.stats(patientData$Genetic_Pedigree_Coefficient)$out  # outlier values.
boxplot(patientData$Genetic_Pedigree_Coefficient,
        main = "Genetic Pedigree",
        boxwex = 1)
mtext(paste(
  "Outliers: ",
  paste(Genetic_Pedigree_Coefficient.Outliers, collapse = ", ")
), cex = 1)

Age.Outliers <-
  boxplot.stats(patientData$Age)$out  # outlier values.
boxplot(patientData$Age, main = "Age", boxwex = 1)
mtext(paste("Outliers: ", paste(Age.Outliers, collapse = ", ")), cex = 1)

BMI.Outliers <-
  boxplot.stats(patientData$BMI)$out  # outlier values.
boxplot(patientData$BMI, main = "BMI", boxwex = 1)
mtext(paste("Outliers: ", paste(BMI.Outliers, collapse = ", ")), cex = 1)

Physical_Activity.Outliers <-
  boxplot.stats(patientData$Physical_activity)$out  # outlier values.
boxplot(patientData$Physical_activity,
        main = "Physical_Activity",
        boxwex = 1)
mtext(paste("Outliers: ", paste(Physical_Activity.Outliers, collapse = ", ")), cex =
        1)

Salt_Content.Outliers <-
  boxplot.stats(patientData$salt_content_in_the_diet)$out  # outlier values.
boxplot(patientData$salt_content_in_the_diet,
        main = "salt_content_in_the_diet",
        boxwex = 1)
mtext(paste("Outliers: ", paste(Salt_Content.Outliers, collapse = ", ")), cex =
        1)

Alcohol_Consumption.Outliers <-
  boxplot.stats(patientData$alcohol_consumption_per_day)$out  # outlier values.
boxplot(patientData$alcohol_consumption_per_day,
        main = "alcohol_consumption_per_day",
        boxwex = 1)
mtext(paste(
  "Outliers: ",
  paste(Alcohol_Consumption.Outliers, collapse = ", ")
), cex = 1)

# Check the Percentage of the Null Values for Each Columns in the Provided Data
na_count = sapply(patientData, function(y)
...
patientData$Genetic_Pedigree_Coefficient[is.na(patientData$Genetic_Pedigree_Coefficient)] <-
  Genetic_Pedigree.Mean
...
patientData$alcohol_consumption_per_day[is.na(patientData$alcohol_consumption_per_day)] <-
  0
...
threshold = 0.6
predicted_values <-
  ifelse(predict(LogisticModel, type = "response") > threshold, 1, 0)
...
DecisionTreeModel <-
  train(
    as.factor(Blood_Pressure_Abnormality) ~ Level_of_Hemoglobin + Genetic_Pedigree_Coefficient,
    data = trainingData,
    method = "rpart",
...
prp(DecisionTreeModel$finalModel,
    box.palette = "Reds",
    tweak = 1.2)
...
print(paste(
  'Logistic  Model Accuracy (in %)',
  logisticRegressionMatrix$overall[1] * 100
))

print(paste(
  'Decision Tree Accuracy (in %)',
  decisionTreeMatrix$overall[1] * 100
))

print(paste(
  'Random Forest Accuracy (in %)',
  randomForestMatrix$overall[1] * 100
))

print(paste('Naive Bayes Accuracy (in %)', naiveBayesMatrix$overall[1] *
              100))
...
accuracy = c(
  ...
  decisionTreeMatrix$overall[1] * 100,
  randomForestMatrix$overall[1] * 100,
  naiveBayesMatrix$overall[1] * 100,
  svmMatrix$overall[1] * 100
)
